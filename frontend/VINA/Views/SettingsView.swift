...
struct SettingsView: View {
    ...
    @AppStorage(StorageStrings.NARRATE_BREAKINGNEWS) var narrateBreakingNews: Bool = false
    @AppStorage(StorageStrings.BACKGROUND_TINT) var backgroundTint: String = Self.DEFAULT_TINT
    @AppStorage(StorageStrings.HISTORY_SIZE) var historySize: Int = Constants.DEFAULT_BUFFER_CAPACITY
        ...
        HStack {
            ...
            Menu(self.backgroundTint.capitalized) {
                ForEach(Theme.allCases) { theme in
                    Button(theme.name, action: { updateBackground(themeName: theme.name)})
